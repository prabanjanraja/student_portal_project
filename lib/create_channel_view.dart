...
import 'package:sendbird_flutter_dashchat/get_channel_name.dart';
...
class _CreateChannelViewState extends State<CreateChannelView> {
  ...
  Future<GroupChannel> createChannel(List<String> userIds, String name) async {
    try {
  ...
  Widget navigationBar() {
    return AppBar(
      ...
      actions: [
        // IconButton(onPressed: onPressed, icon: Icons.create)
        TextButton(
            style: ButtonStyle(
                foregroundColor: MaterialStateProperty.all<Color>(
                    Theme.of(context).buttonColor)),
            onPressed: () async {
              String _channelName = await showDialog<String>(
                  context: context, builder: (_) => get_name());
              ...
              print('Name before calling the create is $_channelName');
              createChannel(
